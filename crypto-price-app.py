# -*- coding: utf-8 -*-
"""
Created on Wed Oct 20 20:57:48 2021
...
def load_data():
    ...
    #coins = {}
    #for i in listings[1:]:
    #coins[i[8]] = i[-5] # id = slug

    coin_df = pd.DataFrame(columns=listings[0]['keysArr'])
    listings_df = pd.DataFrame(listings[1:])
    listings_df = listings_df.iloc[:,:-1]
    listings_df.columns = listings[0]['keysArr']
    coin_df = coin_df.append(listings_df)
    
    
    
    var_names = ['name', 'symbol', 'quote.'+currency_price_unit+'.price',
         ...
         'quote.'+currency_price_unit+'.volume24h']
    df = coin_df[var_names]
    df.columns = ['coin_name', 'coin_symbol', 'price', 'percent_change_1h', 'percent_change_24h', 'percent_change_7d', 'market_cap', 'volume_24h']
